...
public class PluginResolverFactory implements Factory<PluginResolver> {
    ...
    private void addDefaultResolvers(List<PluginResolver> resolvers) {
        ...
        // Use buildSrc scope
        ClassLoaderScope scriptPluginsScope = projectRegistry.getRootProject().getClassLoaderScope().getParent();
        resolvers.add(new ScriptPluginPluginResolver(scriptPluginsScope));

...
public class ScriptPluginPluginResolver implements PluginResolver {
    ...
    public ScriptPluginPluginResolver(ClassLoaderScope targetScope) {
        pluginsLoader = new ScriptPluginLoaderClassLoader();
    ...
    private static class ScriptPluginLoaderClassLoader extends URLClassLoader {
        ...
        private static final Type SCRIPT_PLUGIN_LOADER_LOAD_METHOD = Type.getMethodType(SCRIPT_PLUGIN_TYPE,
            STRING_TYPE, STRING_TYPE, PROJECT_REGISTRY_TYPE, SCRIPT_HANDLER_TYPE, SCRIPT_PLUGIN_FACTORY_TYPE);
        ...
        private ScriptPluginLoaderClassLoader() {
            super(new URL[0]);
        }

        private Class<?> defineScriptPluginLoaderClass(ContextAwarePluginRequest pluginRequest, String displayName) {

            String scriptUriString = pluginRequest.getScriptUri().toString();
            String classSimpleName = loaderClassSimpleNameFor(scriptUriString);
            ...
            defineApplyMethod(cw, syntheticLoaderInternalName, scriptUriString, displayName);
        ...
        private void defineApplyMethod(ClassWriter cw, String syntheticLoaderInternalName, String scriptUriString, String displayName) {
            ...
            apply.visitLdcInsn(scriptUriString);
        ...
        private String loaderClassSimpleNameFor(String resolvedScriptUri) {
            String hash = HashUtil.createHash(resolvedScriptUri, "SHA1").asCompactString();
            return SYNTHETIC_LOADER_CLASSNAME_PREFIX + hash;
        }
    }
}
